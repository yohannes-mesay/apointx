...
import { getAppointmentsByTime } from "@/lib/db/queries";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await getAppointmentsByTime(startDate, endDate, username);

    return NextResponse.json(data);
  } catch (error) {
    console.error("Error fetching appointments by time:", error);
    return NextResponse.json(
      { error: "Failed to fetch appointments by time" },
      { status: 500 }
    );
  }
}
